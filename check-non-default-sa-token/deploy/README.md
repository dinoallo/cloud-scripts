...
The RBAC includes read access to Secrets so the scanner can attribute legacy `kubernetes.io/service-account-token` Secret volumes. Remove the `secrets` rule from `rbac.yaml` and add `--skip-secret-inspection` to the Job or CronJob args if Secret reads are not allowed in your cluster.

Because the scanner runs with its own non-default ServiceAccount, scan results can include the scanner Job or CronJob itself. Treat that finding as expected unless you choose a different runtime identity.
